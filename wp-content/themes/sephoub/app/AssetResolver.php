...
namespace SeaPicture;

/**
 * Class AssetResolver
 *
 * This utility class resolves URIs to built assets in the /public directory of our theme. Built assets have a version
 ...
 * /public/mix-manifest.json without having to specify the version hash.
 *
 * @package SeaPicture
 */
class AssetResolver {
	...
	public static function resolve( $path ) {
		if ( $map = self::get_manifest() ) {
			...
			if ( isset( $map[ $path ] ) ) {
				return get_stylesheet_directory_uri() . '/public' . self::leading_slash_it( $map[ $path ] );
			}
		}
	...
	private static function get_manifest() {
		if ( ! self::$manifest ) {
			$manifest = get_stylesheet_directory() . '/public/mix-manifest.json';
